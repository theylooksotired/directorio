...
class PayPal {
	...
	static public function checkoutButton($options=array()) {
		$options = PayPal::formatOptions($options);
		$image = ($options['image']!='none') ? '<button type="submit" class="buttonPaypal"><span>Pagar mediante</span> <i class="icon icon-paypal"></i></button>' : '';
		return '<form action="'.PAYPAL_URL.'" method="post" class="paypalForm" id="paypalForm">
	...
	static public function checkoutRequestUrl($options=array()) {
		return PAYPAL_URL.'?'.PayPal::formatOptionsUrl($options);
	}

	static public function checkoutRequest($options=array()) {
		$options['image'] = 'none';
		$options = PayPal::formatOptions($options);
		return PayPal::checkoutButton($options).'
	...
	static public function formatOptionsUrl($options=array()) {
		$values = array();
		$values['cmd'] = '_xclick';
		$values['business'] = PAYPAL_ID;
		$values['item_name'] = (isset($options['item_name'])) ? $options['item_name'] : Parms::param('titlePage');
