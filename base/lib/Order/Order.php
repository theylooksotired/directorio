...
class Order extends Db_Object {
	...
	public function paypalButton($options=array()) {
		return PayPal::checkoutButton($this->formatOptions($options));
	}

	public function paypalRequest($options=array()) {
		header('Location: '.PayPal::checkoutRequestUrl($this->formatOptions($options)));
		exit();
	}

	public function formatOptions($options=array()) {
		$returnUrl = (isset($options['returnUrl']) && $options['returnUrl']!='') ? $options['returnUrl'] : url('pagado/'.md5('plasticwebs_pagado'.$this->id()));
		$cancelUrl = (isset($options['cancelUrl']) && $options['cancelUrl']!='') ? $options['cancelUrl'] : url('anulado/'.md5('plasticwebs_anulado'.$this->id()));
